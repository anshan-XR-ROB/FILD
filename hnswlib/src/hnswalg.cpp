...
namespace hnswlib {
		...
		void DoNotCallThisFunction() 
		{
			// build hnsw
			L2Space l2space(0);
			HierarchicalNSW<float> appr_alg_0(&l2space, 0, 0);
			std::string location;
			HierarchicalNSW<float> appr_alg_1(&l2space, location, false, 0);

            std::vector<float> vec = appr_alg_0.getPointByLabel(-1);
		}
    	...
    	template<typename dist_t>
        HierarchicalNSW<dist_t>::HierarchicalNSW(SpaceInterface<dist_t> *s, const std::string &location, bool nmslib, size_t max_elements) {
            loadIndex(location, s, max_elements);
        }

    	template<typename dist_t>
        HierarchicalNSW<dist_t>::HierarchicalNSW(SpaceInterface<dist_t> *s, size_t max_elements, size_t M, size_t ef_construction, size_t random_seed) :
        ...
        HierarchicalNSW<dist_t>::searchBaseLayer(tableint enterpoint_id, void *data_point, int layer) {
            ...
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst<dist_t> > candidateSet;
    	...
    	template<typename dist_t>
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst<dist_t> >
        HierarchicalNSW<dist_t>::searchBaseLayerST(tableint ep_id, const void *data_point, size_t ef) const {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;

            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst<dist_t> > top_candidates;
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst<dist_t> > candidate_set;
		...
		template<typename dist_t>
        void HierarchicalNSW<dist_t>::getNeighborsBySimple(
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst<dist_t> > &top_candidates,
                const size_t M) {
			int len = (int)top_candidates.size() - (int)M;
			while (len-- > 0) top_candidates.pop();
		}
    	...
    	template<typename dist_t>
        linklistsizeint *HierarchicalNSW<dist_t>::get_linklist0(tableint internal_id, char *data_level0_memory_) {
            return (linklistsizeint *) (data_level0_memory_ + internal_id * size_data_per_element_ + offsetLevel0_);
        };
    	...
    	template<typename dist_t>
        void HierarchicalNSW<dist_t>::mutuallyConnectNewElement(void *data_point, tableint cur_c,
                                       std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst<dist_t> > top_candidates,
                                       int level) {

            size_t Mcurmax = level ? maxM_ : maxM0_;
            // getNeighborsByHeuristic2(top_candidates, M_);
            getNeighborsBySimple(top_candidates, M_);
                ...
                if (sz_link_list_other < Mcurmax) {
                    ...
                    *ll_other = sz_link_list_other + 1;
                    ...
                    dist_t d_max = fstdistfunc_(getDataByInternalId(cur_c), getDataByInternalId(selectedNeighbors[idx]),
                                                dist_func_param_);
                    // Heuristic:
                    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst<dist_t> > candidates;
                    ...
                    getNeighborsBySimple(candidates, Mcurmax);
    	...
    	template<typename dist_t>
        std::priority_queue<std::pair<dist_t, tableint>> HierarchicalNSW<dist_t>::searchKnnInternal(void *query_data, int k) {
        ...
        template<typename dist_t>
        void HierarchicalNSW<dist_t>::addPoint(void *data_point, labeltype label)
        {
            addPoint(data_point, label,-1);
        };

    	template<typename dist_t>
        tableint HierarchicalNSW<dist_t>::addPoint(void *data_point, labeltype label, int level) {
            ...
            if ((signed)currObj != -1) {
                ...
                for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                    ...
                    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst<dist_t> > top_candidates = searchBaseLayer(
    	...
    	template<typename dist_t>
        std::vector<float> HierarchicalNSW<dist_t>::getPointByLabel(labeltype label)
        {
            std::vector<float> data;
            if(label_lookup_.count(label) == 0)
            {
                std::cout << " - label lookup error: has no index " << label << std::endl;    
                return data;
            }
            int dim = data_size_ / sizeof(float);
            data.resize(dim);

            tableint cur_c = label_lookup_[label]; 
            memcpy((char*)data.data(), getDataByInternalId(cur_c), data_size_);
            
            return data;
        }

    	template<typename dist_t>
        std::priority_queue<std::pair<dist_t, labeltype >> HierarchicalNSW<dist_t>::searchKnn(const void *query_data, size_t k) const {
            tableint currObj = enterpoint_node_;
            dist_t curdist = fstdistfunc_(query_data, getDataByInternalId(enterpoint_node_), dist_func_param_);
        	...
        	std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst<dist_t> > top_candidates = searchBaseLayerST(
